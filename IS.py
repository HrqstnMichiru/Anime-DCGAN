...
def inception_score(batch_size=8, resize=True, splits=10):
    # Set up dtype
    device = torch.device("cuda:0")  # you can change the index of cuda
    # Load inception model
    inception_model = inception_v3(weights=torchvision.models.Inception_V3_Weights).to(
        device
    )
    inception_model.eval()
    up = nn.Upsample(size=(256, 256), mode="bilinear", align_corners=False).to(device)
